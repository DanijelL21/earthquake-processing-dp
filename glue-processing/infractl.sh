...
case "${EXEC}" in 
    local)
        ...
        if [ "$ACTION" == "destroy" ]; then
            ...
            terraform destroy -var="profile=$PROFILE" -var-file=../terraform.tfvars
        elif [ "$ACTION" == "deploy" ]; then
            echo "Planning the deployment..."
            terraform plan -var="profile=$PROFILE" -var-file=../terraform.tfvars

            echo "Applying the plan..."
            terraform apply -auto-approve -var="profile=$PROFILE" -var-file=../terraform.tfvars
        else
